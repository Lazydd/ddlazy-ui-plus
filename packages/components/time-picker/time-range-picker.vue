...
const { format, valueFormat, order, disabled, open, disabledTime } = defineProps(timeRangePickerProps);
	...
	set(value) {
		...
		if (order) {
			...
			newValue =
				t1 &&
					t2 &&
					dayjs(t2, valueFormat as string).isBefore(
						dayjs(t1, valueFormat as string)
					)
					? newValue.reverse()
					: newValue;
		}
...
<template>
	<div :class="[
	...
	]" ref="timeRangePicker" @click="timeRangePickerClick">
		<TimeInput ref="timePickerInput1" :format-show="formatShow[0]" :format-value="formatValue[0]" :disabled
			:placeholder="placeholder[0]" :suffix="false" :showIcon="false" :allow-clear="false" :readonly
		...
		<TimeInput ref="timePickerInput2" :format-show="formatShow[1]" :format-value="formatValue[1]" :disabled
			:placeholder="placeholder[1]" :allow-clear="false" :show-icon="false" :readonly @click.stop="endClick" />
		<span class="dd-picker-suffix">
			<slot name="suffixIcon">
				<svg focusable="false" data-icon="clock-circle" width="1em" height="1em" fill="currentColor"
		...
		<span class="dd-picker-clear" style="right: 11px" role="button" @click.stop="clearClick"
			v-if="allowClear && formatValue[0] && formatValue[1] && !disabled">
			<svg focusable="false" data-icon="close-circle" width="1em" height="1em" fill="currentColor"
				aria-hidden="true" viewBox="64 64 896 896">
				<path
					d="M512 64C264.6 64 64 264.6 64 512s200.6 448 448 448 448-200.6 448-448S759.4 64 512 64zm165.4 618.2l-66-.3L512 563.4l-99.3 118.4-66.1.3c-4.4 0-8-3.5-8-8 0-1.9.7-3.7 1.9-5.2l130.1-155L340.5 359a8.32 8.32 0 01-1.9-5.2c0-4.4 3.6-8 8-8l66.1.3L512 464.6l99.3-118.4 66-.3c4.4 0 8 3.5 8 8 0 1.9-.7 3.7-1.9 5.2L553.5 514l130 155c1.2 1.5 1.9 3.3 1.9 5.2 0 4.4-3.6 8-8 8z" />
			</svg>
		</span>
		<div class="dd-picker-active-bar" :style="{
		...
		<Teleport to="body">
			<Popover v-if="timeRangePickerRef" v-model:visible="timeRangePickerContainerShow"
				:instance="timeRangePickerRef" :padding="0" @outSideClick="outSideClick"
					...
					<div class="dd-picker-time-panel">
						<div class="dd-picker-content">
							<Time ref="hour" type="hour" :step="hourStep" v-model:value="timeValue"
								:disabled-time="disabledHours()" :active-input @click="changeFormat" />
							<Time ref="minute" type="minute" :step="minuteStep" v-model:value="timeValue"
								:disabled-time="disabledMinutes(timeInfo.hour)" :active-input @click="changeFormat" />
							<Time ref="second" type="second" :step="secondStep" v-model:value="timeValue"
								:disabled-time="disabledSeconds(timeInfo.hour, timeInfo.minute)" :active-input
								@click="changeFormat" />
						</div>
					</div>
					<div class="dd-picker-footer">
						...
						<ul class="dd-picker-ranges">
							...
							<li class="dd-picker-ok">
								<dd-button type="primary" size="small" @click="ok" :disabled="!timeValue[activeInput]">
									确定
								</dd-button>
							</li>
						</ul>
