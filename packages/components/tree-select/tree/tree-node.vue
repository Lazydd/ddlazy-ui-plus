...
const select = (value: string | number, disabled: boolean, node: TreeNodeType) => {
	...
	if (props.multiple) {
		...
		} else props.selectedKeys.set(value, node);
	} else {
...
const checked = (disabled: boolean, node: TreeNodeType) => {
	if (disabled || props.disabled) return;
	props.setNoChecked(false);

	node.isChecked = !node.isChecked;
	if (node.isChecked) node.isHalfChecked = false;

	store.checkedKeys[node.isChecked ? 'add' : 'delete'](node.value);
	console.log(node);
...
const setChildrenCheckedKeys = (node: TreeNodeType, isChecked: boolean) => {
	...
	if (children?.length) {
		children?.forEach((v) => {
			...
			store.checkedKeys[node.isChecked ? 'add' : 'delete'](v.value);
			setChildrenCheckedKeys(v, isChecked);
		});
	}
};
const store = useStore();
const treeMap = computed(() => store.filterTreeMap);
const setParentCheckedKeys = (node: TreeNodeType, isChecked: boolean) => {
	let parentKey = node?.parentKey;
	while (parentKey) {
		const parent = props.isSearch ? treeMap.value.get(parentKey) : props.treeMap.get(parentKey);
		...
		if (isAll) {
			...
			store.checkedKeys[isChecked ? 'add' : 'delete'](parent.value);
		} else {
			parent.isChecked = false;
			store.checkedKeys.delete(parent.value);
...
// 	() => store.checkedKeys,
// 	() => {
// 		store.checkedKeys.forEach((t) => {
// 			setChildrenCheckedKeys(store.treeMap.get(t), true);
// 			setParentCheckedKeys(store.treeMap.get(t), true);
// 		});
// 	},
// 	{
// 		immediate: true,
...
defineExpose({
	setChildrenCheckedKeys,
	setParentCheckedKeys,
	checked,
});
</script>
			...
			>
				<slot
					...
					v-bind="{
						...
						checked: store.checkedKeys.has(data.value),
						switcherCls: {
