...
<template>
	<Teleport :to="toContainer" v-if="toContainer">
		<dialog ref="dialog" v-if="tag" :class="['dd-modal', wrapClassName, { mask }]"
			:style="{ width: width.toString().endsWith('px') ? width : width + 'px', left: x + 'px', top: y + 'px' }">
			<div class="dd-modal-content">
				...
				<div class="dd-modal-footer" v-if="footer !== null">
					<slot name="footer">
						<dd-button plain @click="(e: MouseEvent) => cancel(e)" v-bind="cancelButtonProps">
							{{ cancelText }}
						</dd-button>
						<dd-button :type="okType" :loading="confirmLoading" @click="(e: MouseEvent) => ok(e)"
