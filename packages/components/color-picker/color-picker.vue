...
import { colorPickerProps, ColorPickerProps } from './types';
...
const finalPresetColors = computed(() => presets);
...
const displayColor = computed(() => {
	...
	const baseColor = modelValue.value || color;
...
onMounted(() => {
	const initialColor = modelValue.value || color;
	if (!initialColor) {
		return;
	}
...
const formatOutputColor = (color: string, alphaValue: number): string => {
	...
	if (outputFormat === 'auto') {
		...
		// 根据输入值确定格式
		if (modelValue.value) {
			if (isRgb(modelValue.value)) {
				outputFormat = 'rgb';
			} else if (isHsl(modelValue.value)) {
				outputFormat = 'hsl';
			} else {
				outputFormat = 'hex';
			}
